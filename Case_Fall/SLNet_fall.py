...
W = 251                             # <====
wind_type = 'gaussian'              # <====
use_fused_spec_and_filter = False    # <==== False
use_singl_spec_and_filter = False    # <==== False
downsamp_ratio_for_stft = 100        # 1000Hz / 10 = 10Hz        # <====
data_dir = 'xxx/' # <====
str_SLNet_name = 'SLNet_fall_torch.pt'
n_epoch = 50    # <====
# --------------------------
epoch_filter = 5000
W_all = [125,251,501]
...
def normalize_data(data_1):
    ...
    # data(ndarray.complex)=>data_norm(ndarray.complex): [R,1,121,T]=>[R,1,121,T]
    data_1_abs = abs(data_1)
    data_1_max = data_1_abs.max(axis=(2,3),keepdims=True)     # [R,1,121,T]=>[R,1,1,1]
    data_1_max_rep = np.tile(data_1_max,(1,1,data_1_abs.shape[2],data_1_abs.shape[3]))    # [R,1,1,1]=>[R,1,121,T]
    data_1_norm = data_1 / data_1_max_rep   # [R,1,121,T]
    return  data_1_norm
...
def load_data_to_array(path_to_data):
        ...
        for data_file_name in data_files:
            ...
            try:
                ...
                if use_fused_spec_and_filter:
                    ...
                    data_1 = np.expand_dims(data_1, axis=0)         # [1,121,t]=>[1,1,121,t]
                ...
                # Normalization
                data_normed_1 = normalize_data(data_1)  # [R,C,F,T] ~ [R,1,121,t]
    ...
    data = complex_array_to_2_channel_float_array(data)    # [N,R,C,F,T]~[N,R,1,121,T]=>[N,2,R,1,121,T]
...
class SLNet(nn.Module):
    ...
    def __init__(self, input_shape, n_class):
        ...
        self.input_shape = input_shape  # [2,R,1,121,T_MAX]
        ...
        self.fc_4 = nn.Linear(1*(self.T_MAX-8)*32, 256)
    ...
    def forward(self, x):
        ...
        h = relu(self.fc_3(h))                      # (@,1,T_MAX-8,64)=>(@,1,T_MAX-8,32)
        h = h.reshape((-1,1*(self.T_MAX-8)*32))     # (@,1,T_MAX-8,32)=>(@,1*(T_MAX-8)*32)
        h = self.dropout_3(h)
        h = relu(self.fc_4(h))                      # (@,1*(T_MAX-8)*32)=>(@,256)
        h = self.dropout_4(h)
...
if use_existing_SLNet_model:
    ...
    torch.save(model, str_SLNet_name)
    print('Model trained...')
