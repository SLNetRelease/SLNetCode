...
W = 251                             # <====
wind_type = 'gaussian'              # <====
downsamp_ratio_for_stft = 40        # 1000Hz / 40 = 25Hz        # <====
data_dir = 'xxx/'
mat_file_key = 'DFS'
epoch_filter = 5000
str_SLNet_name = 'xxx.pt'
...
def complex_array_to_2_channel_float_array(data_complex):
    # data_complex(complex128/float64)=>data_float: [N,1,121,T_MAX]=>[N,2,1,121,T_MAX]
...
def load_data_to_array(path_to_data):
    ...
    for data_root, data_dirs, data_files in os.walk(path_to_data):
        for data_file_name in data_files:
            ...
            try:
                # Label embedding
                label_1_str = data_file_name.split('_')[0]
                if label_1_str == 'fall':
                    label_1 = 1
                elif label_1_str == 'nonfall':
                    label_1 = 0
                else:
                    print('Wrong fall/nonfall prefix, skiping...')
                    continue
                domain_1_name = data_file_name.split('_')[2]
                domain_1 = ID_LIST.index(domain_1_name)
            ...
            domain.append(domain_1)

    # Zero-padding
    data = zero_padding(data, T_MAX)    # ([6,121,t1],...)=>[N,1,121,T]

    # Convert from complex128 to 2_channel_float32
    data = complex_array_to_2_channel_float_array(data)    # [N,1,121,T]=>[N,2,1,121,T]

    # Convert label to ndarray
    label = np.array(label)
    domain = np.array(domain)

    # data(ndarray): [N,2,1,121,T_MAX], label(ndarray): [N,], domain(ndarray): [N,]
...
def load_data_to_loader(data_dir):
    ...
    n_domain_loaded = np.unique(domain).shape[0]
    ...
    print('Loaded {} samples, split {}/{}/{} for train/valid/test, data shape: {}, data dtype: {}, class num (loaded/expected): {}/{}'.format(\
        sample_count,train_sample_count,valid_sample_count,test_sample_count,sample_shape,sample_dtype, n_class_loaded, 2))
    return train_data_loader, valid_data_loader, test_data_loader, sample_shape, n_domain_loaded
...
class Baseline_Widar3(nn.Module):
    # Need customization
    def __init__(self, input_shape, class_num, domain_num):
        super(Baseline_Widar3, self).__init__()
        self.input_shape = input_shape  # [2,1,121,T_MAX]
        ...
        self.GRU = nn.GRU(input_size=1*32, hidden_size=128, num_layers=1, bias=True, batch_first=True, dropout = 0.5)

    def forward(self, x):
        h = x   # (@,2,1,121,T_MAX)

        # Baseline: CNN+GRU
        h = torch.linalg.norm(h,dim=1)          # (@,2,1,121,T_MAX)=>(@,1,121,T_MAX)
        h = torch.unsqueeze(h,dim=1)            # (@,1,121,T_MAX)=>(@,1,1,121,T_MAX)

        h = relu(self.conv3d_1(h))              # (@,1,1,121,T_MAX)=>(@,16,1,117,T_MAX-4)
        h = self.mpooling3d_1(h)                # (@,16,1,117,T_MAX-4)=>(@,16,1,39,T_MAX-4)

        h = relu(self.conv3d_2(h))              # (@,16,1,39,T_MAX-4)=>(@,32,1,35,T_MAX-8)
        h = self.mpooling3d_2(h)                # (@,32,1,35,T_MAX-8)=>(@,32,1,7,T_MAX-8)

        h = relu(self.conv3d_3(h))              # (@,32,1,7,T_MAX-8)=>(@,32,1,5,T_MAX-10)

        h = h.permute(0,2,4,1,3)                # (@,32,1,5,T_MAX-10)=>(@,1,T_MAX-10,32,5)
        h = h.reshape((-1,1,self.T_MAX-10,32*5))# (@,1,T_MAX-10,32,5)=>(@,1,T_MAX-10,32*5)
        h = self.dropout_1(h)
        h = relu(self.fc_1(h))                  # (@,1,T_MAX-10,32*5)=>(@,1,T_MAX-10,128)
        h = self.dropout_2(h)
        h = relu(self.fc_2(h))                  # (@,1,T_MAX-10,128)=>(@,1,T_MAX-10,64)
        h = relu(self.fc_3(h))                  # (@,1,T_MAX-10,64)=>(@,1,T_MAX-10,32)

        h = h.permute(0,2,1,3)                  # (@,1,T_MAX-10,32)=>(@,T_MAX-10,1,32)

        h = h.reshape((-1,self.T_MAX-10,1*32))  # (@,T_MAX-10,1,32)=>(@,T_MAX-10,1*32)
        
        h,_ = self.GRU(h)                       # (@,T_MAX-10,1*32)=>(@,T_MAX-10,128)
...
class Baseline_EI(nn.Module):
    ...
    def __init__(self, input_shape, class_num, domain_num):
        ...
        self.T_MAX = input_shape[3]
        self.class_num = class_num
        self.n_domain = domain_num

        # CNN+FC
        self.fc_1 = nn.Linear(32*5, 128)
        self.fc_2 = nn.Linear(128, 64)
        self.fc_3 = nn.Linear(64, 32)
        self.fc_out_1_1 = nn.Linear(1*(self.T_MAX-10)*32, 256)
        self.fc_out_1_2 = nn.Linear(256, self.class_num)
        self.fc_out_2_1 = nn.Linear(1*(self.T_MAX-10)*32, 256)
        self.fc_out_2_2 = nn.Linear(256, self.n_domain)    # 25 domains
        self.dropout_1 = nn.Dropout(p=0.2)
        self.dropout_2 = nn.Dropout(p=0.3)
        self.dropout_3 = nn.Dropout(p=0.4)
        self.conv3d_1 = nn.Conv3d(in_channels= 1, out_channels=16, kernel_size=[1,5,5], stride=[1,1,1])
    ...
    def forward(self, x):
        ...
        h = relu(self.fc_2(h))                  # (@,1,T_MAX-10,128)=>(@,1,T_MAX-10,64)
        h = relu(self.fc_3(h))                  # (@,1,T_MAX-10,64)=>(@,1,T_MAX-10,32)
        h = h.reshape((-1,1*(self.T_MAX-10)*32)) # (@,1,T_MAX-10,32)=>(@,1*(T_MAX-10)*32)
        h = self.dropout_3(h)

        # Branch-1: Gesture class
        output_1 = relu(self.fc_out_1_1(h))
        output_1 = self.fc_out_1_2(output_1)        # (@,1*(T_MAX-8)*32)=>(@,n_class)  (No need for activation when using CrossEntropyLoss)

        # Branch-2: Domain
        output_2 = relu(self.fc_out_2_1(h))
        output_2 = self.fc_out_2_2(output_2)        # (@,1*(T_MAX-8)*32)=>(@,n_domain)
...
class Baseline_CrossSense(nn.Module):
    ...
    def forward(self, x):
        ...
        # Baseline: ANN
        h = torch.linalg.norm(h,dim=1)          # (@,2,1,121,T_MAX)=>(@,1,121,T_MAX)
        h = h.permute(0,3,1,2)                  # (@,1,121,T_MAX)=>(@,T_MAX,1,121)

        h = h.reshape((-1,self.T_MAX,1*121))    # (@,T_MAX,1,121)=>(@,T_MAX,1*121)
        h = self.dropout_1(h)
        h = relu(self.fc_1(h))                  # (@,T_MAX,1*121)=>(@,T_MAX,512)
        h = self.dropout_2(h)
...
class Baseline_RFSleep(nn.Module):
    ...
    def __init__(self, input_shape, class_num, domain_num):
        ...
        self.fc_out_1_2 = nn.Linear(128, self.class_num)
        self.fc_out_2_1 = nn.Linear((self.T_MAX-10)*128, 128)
        self.fc_out_2_2 = nn.Linear(128, self.n_domain)    # 25 domains
        self.dropout_1 = nn.Dropout(p=0.2)
        self.dropout_2 = nn.Dropout(p=0.3)
        self.dropout_3 = nn.Dropout(p=0.4)
        self.conv3d_1 = nn.Conv3d(in_channels= 1, out_channels=16, kernel_size=[1,5,5], stride=[1,1,1])
...
class Baseline_RFPose(nn.Module):
    ...
    def __init__(self, input_shape, class_num, domain_num):
        ...
        self.fc_out_1 = nn.Linear(1*(self.T_MAX-10)*64, 256)
    ...
    def forward(self, x):
        ...
        # Baseline: CNN+FC
        h_1 = h[:,0,:,:,:]                          # (@,2,1,121,T_MAX)=>(@,1,121,T_MAX)
        h_1 = torch.unsqueeze(h_1,dim=1)            # (@,1,121,T_MAX)=>(@,1,1,121,T_MAX)
        h_1 = relu(self.conv3d_1_1(h_1))            # (@,1,1,121,T_MAX)=>(@,16,1,117,T_MAX-4)
        h_1 = self.mpooling3d_1_1(h_1)              # (@,16,1,117,T_MAX-4)=>(@,16,1,39,T_MAX-4)
        h_1 = relu(self.conv3d_2_1(h_1))            # (@,16,1,39,T_MAX-4)=>(@,32,1,35,T_MAX-8)
        h_1 = self.mpooling3d_2_1(h_1)              # (@,32,1,35,T_MAX-8)=>(@,32,1,7,T_MAX-8)
        h_1 = relu(self.conv3d_3_1(h_1))            # (@,32,1,7,T_MAX-8)=>(@,32,1,5,T_MAX-10)
        h_1 = h_1.permute(0,2,4,1,3)                # (@,32,1,5,T_MAX-10)=>(@,1,T_MAX-10,32,5)
        h_1 = h_1.reshape((-1,1,self.T_MAX-10,32*5))# (@,1,T_MAX-10,32,5)=>(@,1,T_MAX-10,32*5)
        h_1 = self.dropout_1_1(h_1)
        h_1 = relu(self.fc_1_1(h_1))                  # (@,1,T_MAX-10,32*5)=>(@,1,T_MAX-10,128)
        h_1 = self.dropout_2_1(h_1)
        h_1 = relu(self.fc_2_1(h_1))                  # (@,1,T_MAX-10,128)=>(@,1,T_MAX-10,64)
        h_1 = relu(self.fc_3_1(h_1))                  # (@,1,T_MAX-10,64)=>(@,1,T_MAX-10,32)

        h_2 = h[:,1,:,:,:]                          # (@,2,1,121,T_MAX)=>(@,1,121,T_MAX)
        h_2 = torch.unsqueeze(h_2,dim=1)            # (@,1,121,T_MAX)=>(@,1,1,121,T_MAX)
        h_2 = relu(self.conv3d_1_2(h_2))            # (@,1,1,121,T_MAX)=>(@,16,1,117,T_MAX-4)
        h_2 = self.mpooling3d_1_2(h_2)              # (@,16,1,117,T_MAX-4)=>(@,16,1,39,T_MAX-4)
        h_2 = relu(self.conv3d_2_2(h_2))            # (@,16,1,39,T_MAX-4)=>(@,32,1,35,T_MAX-8)
        h_2 = self.mpooling3d_2_2(h_2)              # (@,32,1,35,T_MAX-8)=>(@,32,1,7,T_MAX-8)
        h_2 = relu(self.conv3d_3_2(h_2))            # (@,32,1,7,T_MAX-8)=>(@,32,1,5,T_MAX-10)
        h_2 = h_2.permute(0,2,4,1,3)                # (@,32,1,5,T_MAX-10)=>(@,1,T_MAX-10,32,5)
        h_2 = h_2.reshape((-1,1,self.T_MAX-10,32*5))# (@,1,T_MAX-10,32,5)=>(@,1,T_MAX-10,32*5)
        h_2 = self.dropout_1_2(h_2)
        h_2 = relu(self.fc_1_2(h_2))                  # (@,1,T_MAX-10,32*5)=>(@,1,T_MAX-10,128)
        h_2 = self.dropout_2_2(h_2)
        h_2 = relu(self.fc_2_2(h_2))                  # (@,1,T_MAX-10,128)=>(@,1,T_MAX-10,64)
        h_2 = relu(self.fc_3_2(h_2))                  # (@,1,T_MAX-10,64)=>(@,1,T_MAX-10,32)

        h = torch.cat((h_1,h_2),dim=3)              # (@,1,T_MAX-10,32)=>(@,1,T_MAX-10,64)
        h = h.reshape(-1,1*(self.T_MAX-10)*64)      # (@,1,T_MAX-10,64)=>(@,1*(T_MAX-10)*64)

        # Branch-1: Gesture class
        h = relu(self.fc_out_1(h))          # (@,1*(T_MAX-10)*64)=>(@,256)
...
class Baseline_RNN(nn.Module):
    ...
    def __init__(self, input_shape, class_num, domain_num):
        ...
        self.GRU = nn.GRU(input_size=1*121, hidden_size=128, num_layers=3, bias=True, batch_first=True, dropout = 0.2)
    ...
    def forward(self, x):
        ...
        h,_ = self.GRU(h)                       # (@,T_MAX,1*121)=>(@,T_MAX,128)
...
class Baseline_CMLP(nn.Module):
    ...
    def __init__(self, input_shape, class_num, domain_num):
        ...
        # cMLP+rMLP
        self.complex_fc_1 = m_Linear(1*121*self.T_MAX, 2048)
    ...
    def forward(self, x):
        ...
        # Baseline: cMLP+rMLP
        h = h.reshape((-1,2,1*121*self.T_MAX))    # (@,2,1,121,T_MAX)=>(@,2,1*121*T_MAX)

        h = relu(self.complex_fc_1(h))          # (@,2,1*121*T_MAX)=>(@,2,2048)
        h = self.dropout_2(h)
...
class Baseline_CCNN(nn.Module):
    ...
    def __init__(self, input_shape, class_num, domain_num):
        ...
        self.fc_1 = nn.Linear(1*(self.T_MAX-8)*64, 256)
    ...
    def forward(self, x):
        h = x   # (@,2,1,121,T_MAX)
        h = h.unsqueeze(dim=2)                      # (@,2,1,121,T_MAX)=>(@,2,R,1,121,T_MAX)

        # cCONV+cMLP+rMLP
        h = self.pconv3d_1(h)                       # (@,2,R,1,121,T_MAX)=>(@,2,16,1,117,T_MAX-4)
        h = h.reshape((-1,16,1,117,self.T_MAX-4))   # (@,2,16,1,117,T_MAX-4)=>(@*2,16,1,117,T_MAX-4)
        h = self.mpooling3d_1(h)                    # (@*2,16,1,117,T_MAX-4)=>(@*2,16,1,39,T_MAX-4)
        h = h.reshape((-1,2,16,1,39,self.T_MAX-4))  # (@*2,16,1,39,T_MAX-4)=>(@,2,16,1,39,T_MAX-4)

        h = self.pconv3d_2(h)                       # (@,2,16,1,39,T_MAX-4)=>(@,2,32,1,35,T_MAX-8)
        h = h.reshape((-1,32,1,35,self.T_MAX-8))    # (@,2,32,1,35,T_MAX-8)=>(@*2,32,1,35,T_MAX-8)
        h = self.mpooling3d_2(h)                    # (@*2,32,1,35,T_MAX-8)=>(@*2,32,1,7,T_MAX-8)
        h = h.reshape((-1,2,32,1,7,self.T_MAX-8))   # (@*2,32,1,7,T_MAX-8)=>(@,2,32,1,7,T_MAX-8)

        # cMLP
        h = h.permute(0,3,5,1,2,4)                  # (@,2,32,1,7,T_MAX-8)=>(@,1,T_MAX-8,2,32,7)
        h = h.reshape((-1,1,self.T_MAX-8,2,32*7))   # (@,1,T_MAX-8,2,32,7)=>(@,1,T_MAX-8,2,32*7)
        h = self.dropout_1(h)
        h = self.complex_fc_1(h)                    # (@,1,T_MAX-8,2,32*7)=>(@,1,T_MAX-8,2,128)
        h = self.dropout_2(h)
        h = self.complex_fc_2(h)                    # (@,1,T_MAX-8,2,128)=>(@,1,T_MAX-8,2,64)

        # rMLP
        h = torch.linalg.norm(h,dim=3)              # (@,1,T_MAX-8,2,64)=>(@,1,T_MAX-8,64)
        h = h.reshape((-1,1*(self.T_MAX-8)*64))     # (@,1,T_MAX-8,64)=>(@,1*(T_MAX-8)*64)
        h = self.dropout_3(h)
        h = relu(self.fc_1(h))                      # (@,1*(T_MAX-8)*64)=>(@,256)
        h = self.dropout_4(h)
...
train_data_loader, valid_data_loader, test_data_loader, sample_shape, n_domain = load_data_to_loader(data_dir=data_dir)
...
if use_existing_SLNet_model:
    ...
    model = Baseline_CCNN(input_shape=sample_shape, class_num=2, domain_num=n_domain)
