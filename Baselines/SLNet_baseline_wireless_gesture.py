...
def zero_padding(data, T_MAX):
    # data(list)=>data_pad(ndarray): ([6,121,t1/t2/...],...)=>[N,6,121,T_MAX]
    data_pad = []
    ...
    for data_root, data_dirs, data_files in os.walk(path_to_data):
        for data_file_name in data_files:
            ...
            try:
                ...
                domain_1 = 5*(int(data_file_name.split('-')[2])-1) + int(data_file_name.split('-')[3])
                ...
                # Normalization
                data_normed_1 = normalize_data(data_1)  # [C,F,T] ~ [6,121,t]
            ...
            label.append(label_1-1)     # [1,2,...,10]=>[0,1,...,9]
            domain.append(domain_1-1)   # [1,2,...,25]=>[0,1,...,24]

    # Zero-padding
    data = zero_padding(data, T_MAX)    # ([C,F,t],...)=>[N,C,F,T]

    # Convert from complex128 to 2_channel_float32
    data = complex_array_to_2_channel_float_array(data)    # [N,C,F,T]~[N,6,121,T]=>[N,2,6,121,T]

    # Convert label/domain to ndarray
...
class Baseline_Widar3(nn.Module):
    ...
    def __init__(self, input_shape, class_num):
        ...
        self.GRU = nn.LSTM(input_size=6*32, hidden_size=128, num_layers=1, bias=True, batch_first=True, dropout = 0.5)
...
if use_existing_SLNet_model:
    ...
    model = Baseline_CCNN(input_shape=sample_shape, class_num=N_MOTION)
